...
function openC(i){ai=i;const c=D[i];document.querySelectorAll('.ci').forEach(el=>el.classList.toggle('active',parseInt(el.dataset.i)===i));document.getElementById('ch').innerHTML=\`<div class="cname">\${esc(c.name)}</div>\${c.g?\`<div style="font-size:11px;color:var(--t3);font-family:'DM Mono',monospace">\${c.p.length} members</div>\`:''}<div class="csub"><span>\${fmD(c.f)} – \${fmD(c.l)}</span><span>\${c.n.toLocaleString()} msgs</span></div>\`;cm=c.m;rMsgs(c.m);uStats(c);}
function rMsgs(msgs){const el=document.getElementById('msgs');el.innerHTML='';if(!msgs.length){el.innerHTML='<div class="empty"><div class="eg"></div><div class="el">no messages</div></div>';return}let ld=null,lu=null;const f=document.createDocumentFragment();msgs.forEach(m=>{const ds=new Date(m.t).toDateString();if(ds!==ld){const s=document.createElement('div');s.className='dsep';s.textContent=fmDS(m.t);f.appendChild(s);ld=ds;lu=null}const own=m.u===OW,show=m.u!==lu;lu=m.u;const isStat=m.k.startsWith('STATUS_');const r=document.createElement('div');r.className='mr'+(own?' own':'');if(isStat){r.innerHTML=\`<div style="width:100%;text-align:center"><span class="bub stat">\${bText(m)}</span><div class="mtime" style="text-align:center">\${fmT(m.t)}</div></div>\`}else{r.innerHTML=\`<div class="mav">\${own?'◎':ini(m.u)}</div><div class="mc">\${show&&!own?\`<div class="msender">\${esc(m.u)}</div>\`:''}<div class="bub \${bClass(m.k)}">\${bText(m)}</div><div class="mtime">\${fmT(m.t)}</div></div>\`}f.appendChild(r);});el.appendChild(f);el.scrollTop=el.scrollHeight;}
...
let st2=null;function search(q){clearTimeout(st2);st2=setTimeout(()=>{if(!q.trim()||ai===null){if(ai!==null)rMsgs(cm);return}const ql=q.toLowerCase();const f=cm.filter(m=>(m.x&&m.x.toLowerCase().includes(ql))||m.u.toLowerCase().includes(ql));rMsgs(f);const ch=document.getElementById('ch');const ex=ch.querySelector('.csub');if(ex){const sp=document.createElement('span');sp.style.color='var(--y)';sp.textContent=f.length+' matches';ex.appendChild(sp)}},200);}
