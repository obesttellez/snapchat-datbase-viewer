...
function buildViewerHTML(owner, convos, stats) {
  ...
  const dateRange = (() => {
    ...
    for (const c of convos) { if (c.f < min) min = c.f; if (c.l > max) max = c.l; }
...
function rowHTML(item){if(item.type==='sep')return \`<div class="dsep">\${esc(item.label)}</div>\`;const{m,own,show,isStat}=item;if(isStat)return \`<div class="mr"><div style="width:100%;text-align:center"><span class="bub stat">\${bText(m)}</span><div class="mtime" style="text-align:center">\${fmT(m.t)}</div></div></div>\`;const col=uCol(m.u);const cs=(theme==='web'||theme==='app')?\`--send-col:\${col};\`:'';let sh='';if(show){if(theme==='web'||theme==='app'){sh=\`<div class="msender" style="\${cs}">\${own?'Me':esc(m.u)}</div>\`;}else if(!own){sh=\`<div class="msender">\${esc(m.u)}</div>\`;}}return \`<div class="mr\${own?' own':''}" style="\${cs}"><div class="mav">\${own?'◎':ini(m.u)}</div><div class="mc">\${sh}<div class="bub \${bClass(m.k)}" style="\${cs}">\${bText(m)}</div><div class="mtime">\${fmT(m.t)}</div></div></div>\`;}
